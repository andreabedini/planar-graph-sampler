...
public class ConnectedMap extends PlanarMap {
	...
	/**
	 * This is the block decomposition.
	 * it uses the Boltzmann generator dB (pointed biconnected)
	 * and itself (dC, pointed connected)
	 */
        ...
        if (c == 0) {
            ...
            for (int i = 0; i < c; ++i) {
                ...
                for (int k = 0; k < nr_vertices; k++) {
                    ...
                    if (k != j) {
                        twoConnectedMap.replaceVertexByConnectedMap(e, draw_dC(r), r);
                    }
                }
    ...
    public static ConnectedMap draw_ddC(Random r) {
        ...
        for (int i = 0; i < twoConnectedMap.vertexList.size(); ++i) {
            if (i != j) {
                twoConnectedMap.replaceVertexByConnectedMap(twoConnectedMap.vertexList.getDemiArete(i), draw_dC(r), r);
            } else {
                int c = ch_dC_or_ddC.choose(r);
                ConnectedMap connectedMap = new ConnectedMap();
                if (c == 0) {
                    connectedMap = draw_dC(r);
                } else {
                    connectedMap = draw_ddC(r);
                }
    ...
    public static ConnectedMap draw_dddC(Random r) {
        ...
        if (c == 0) {
            ...
            int p = r.nextInt(twoConnectedMap.vertexList.size() - 1);
            if (p >= k) {
                ++p;
            }
            for (int i = 0; i < twoConnectedMap.vertexList.size(); ++i) {
                ...
                if ((i != k) && (i != p)) {
                    ...
                    if (i == k) {
                        ...
                        if (d == 0) {
                            ...
                            System.out.println("pb ch_dC_or_ddC");
                        }
                        twoConnectedMap.replaceVertexByConnectedMap(twoConnectedMap.vertexList.getDemiArete(i), connectedMap, r);
                    } else {
                        twoConnectedMap.replaceVertexByConnectedMap(twoConnectedMap.vertexList.getDemiArete(i), draw_ddC(r), r);
                    }
                }
            }
            return twoConnectedMap;
        } else {
            System.out.println("pb choose_vector dddC");
            return null;
        }
    }
}
